...
const REVALIDATE_SECRET_KEY = 'revalidate_webhook_secret';

const generateSecret = () => uuidv4().replace(/-/g, '_');
const previewSecret = process.env.PREVIEW_SECRET || generateSecret();
const revalidateSecret =
  process.env.REVALIDATE_WEBHOOK_SECRET || generateSecret();
      ...
      if (err) {
        ...
        fs.copyFile(examplePath, envPath, (err: Error | null) => {
          ...
          const updatedEnv = currentEnv
            ...
            .replace(REVALIDATE_SECRET_KEY, revalidateSecret);
